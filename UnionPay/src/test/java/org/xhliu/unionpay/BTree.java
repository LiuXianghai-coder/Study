...
public class BTree<Key extends Comparable<Key>, Value> {
    ...
    private Entry delete(Node parent, Node cur, Key key, int h) {
        ...
        if (h == height) {
            ...
            // idx > cur.m 说明待删除的元素在节点最后的一个元素区间内，同样地，通过递归的方式进行删除
            if (idx > cur.m)
    ...
    private void reBalance(Node parent, Node cur, int h) {
        ...
        */
        ...
        if (left != null) {
            ...
            parent.m--; // 注意这里父节点的元素个数经过合并之后已经减少了
            return;
        }
        ...
        /*
            注意这里根节点的变化，只有在树的高度为 1 并且这一层的所有节点都合并成为一个节点时，
            才能替换掉根节点，此时树的高度 -1
         */
        if (parent.m == 0 && h == 1) {
            root = cur;
    ...
    private Node split(Node x) {
        ...
        // 注意这里的中间节点的 next 链接需要保存到 t 的哨兵元素节点
        t.children[0].next = x.children[M / 2 + 1].next;
